...
SELECT MAX(total_laid_off) as 'Maximum Amount of Total Laid Off', MAX(percentage_laid_off) as 'Maximum Laid Off Percentage' FROM layoff_staging2;
...
SELECT MIN(`date`) as 'Start Date', MAX(`date`) as 'End Date' FROM layoff_staging2;

-- CHECK THE HIGHEST LAID OFF COMPANY THROUGH OUT THE PERIOD
SELECT company, SUM(total_laid_off) as 'Total Laid Off' FROM layoff_staging2 GROUP BY company ORDER BY 2 DESC;

-- CHECK WHICH INDUSTRY IS THE MOST LAID OFF 
SELECT industry, SUM(total_laid_off) as 'Total Laid Off' FROM layoff_staging2 GROUP BY industry ORDER BY 2 DESC;

-- CHECK WHICH COUNTRY IS THE MOST LAID OFF 
SELECT country, SUM(total_laid_off) as 'Total Laid Off' FROM layoff_staging2 GROUP BY country ORDER BY 2 DESC;

-- CHECK WHICH YEAR HAS THE HIGHEST LAID OFF
SELECT YEAR(`date`), SUM(total_laid_off) as 'Total Laid Off' FROM layoff_staging2 GROUP BY YEAR(`date`) ORDER BY YEAR(`date`) DESC;

-- CHECK HIGHEST LAID OFF MONTH AND ROLLING SUM
SELECT SUBSTRING(`date`, 6, 2) as `MONTH`, SUM(total_laid_off) as 'Total Laid Off' FROM layoff_staging2 GROUP BY `MONTH` ORDER BY 1;

-- Query to calculate monthly layoffs and their cumulative rolling sum over time.
...
SELECT company, YEAR(`date`), SUM(total_laid_off) as 'Total Laid Off' FROM layoff_staging2 GROUP BY company, YEAR(`date`) ORDER BY 3 DESC;

-- Query to find the top 5 companies with the highest layoffs per year, ranked by total layoffs.
